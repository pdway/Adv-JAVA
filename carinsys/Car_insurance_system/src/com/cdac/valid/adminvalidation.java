package com.cdac.valid;

import org.springframework.stereotype.Service;
import org.springframework.validation.Errors;
import org.springframework.validation.ValidationUtils;
import org.springframework.validation.Validator;

import com.cdac.dto.Admin;


@Service
public class adminvalidation implements Validator {

	@Override
	public boolean supports(Class<?> clazz) {
		return clazz.equals(Admin.class);
	}

	@Override
	public void validate(Object target, Errors errors) {
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "username","unmKey", "Admin name required");
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "password", "passKey","password required");
		
		Admin user = (Admin)target;
		if(user.getPassword()!=null) {
			if(user.getPassword().length()<3) { 
				errors.rejectValue("password", "passKey", "password should contain more 2 chars");
			}
		}
		
	}

}
